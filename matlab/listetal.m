...
function [output]=listetal(Y,sub,D,combo,select)

% listetal considers the multiple hypothesis testing problem in
% experiment economics discribed in List et al. (2015).
...
% the hypothesis for the ith outcome, jth subgroup, and the kth parwise comparison, and the ijkth element is equal to 0 otherwise.
...
psin=alphasin; % p-values based on the single testing procedure described in Remark 3.1 of List et al. (2015)
...
statsall=zeros(nh,8+B);  % columns 1-5 present the id's of the hypotheses, outcomes, subgroups, and treatment (control) groups;
                         ...
                         % the 7th column presents p-values based on the single testing procedure described in Remark 3.1 of List et al. (2015);
